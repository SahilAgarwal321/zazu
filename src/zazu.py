r"""
Twitter bot in Python3 for populating one's timeline with texts from a text file.
...
def process_config(config_location):
    """
    Check for config file - Raises error if not found.
    Read config file
    """
...
def enable_logging(config):
    """enable logging (checks config file setting)"""
    logfilename = config.get('general', 'log_file_name')
    logging.basicConfig(
        format='%(asctime)s %(levelname)s: zazu %(message)s',
...
def post_tweet(config, tweettext):
    ...
    logging.info(
        "sleeping for %d seconds (%.2f minutes) before posting tweet", randomtime, randomtime / 60
        )
